...
from desk.reputation import install_schema as install_reputation_schema
...
def pool(database_url: str) -> Iterator[ConnectionPool]:
    ...
    with ConnectionPool(database_url, min_size=1, max_size=8, open=True) as pool:
        with pool.connection() as conn:
            ...
            conn.execute("DROP TABLE IF EXISTS agent_reputation")
            ...
            install_reputation_schema(conn)
        yield pool
